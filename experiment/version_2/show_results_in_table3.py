...
File_Head = '/Users/near/NearXu/result/different_headway/dr_result'
...
def show_information():
    ...
    table = PrettyTable(['time', 'scenario', 'headway', 'type', 'precision', 'recall'])

    for headway in different_headway:
        for type in types:
            time = default_start_time
            scenario = default_scenario
            result_json = get_results(type, time, scenario, default_packet_loss_rate, headway)
            precision = result_json['precision']
            recall = result_json['recall']
            table_row = []
            table_row.append(time)
            table_row.append(scenario)
            table_row.append(headway)
