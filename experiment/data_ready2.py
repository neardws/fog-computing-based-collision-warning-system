...
class data_ready:
    ...
    def update_send_packet_plr(self, packet_loss_rate):
        for packets in self.send_packets:
            for vehicle in packets:
    ...
    def get_send_packets(self, time):
        index = time - self.get_start_time(self.time)
        return self.send_packets[index]
