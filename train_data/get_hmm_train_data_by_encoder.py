...
def main():
    with open(HMM_TRAIN_DATA_PATH + '0.txt', 'a+', encoding='utf-8') as hmm_train_file:
                ...
                for xy in xys:
                    if ',' in xy:
                        try:
                            ...
                            x_input = np.expand_dims(np.array(input_normal), axis=0)
                            print(x_input)
                            encode = encoder.predict(x_input)
                            print(encode[0][0])
                            hmm_train_file.writelines(str(encode[0][0]) + ' ')
                        except ValueError:
                            print('Value Error')
